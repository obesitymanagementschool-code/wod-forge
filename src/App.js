...
function GeneratorWizard({onGenerate}) {
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:22}}>
    ...
    <button onClick={()=>onGenerate({format,energy,duration,level,equipment,maxExercises:maxEx,userExercises:userEx,useCustomExercises:useCustomEx})} style={{width:"100%",padding:"14px 0",background:"linear-gradient(90deg,#FF6B35,#FF2D55)",border:"none",borderRadius:11,fontFamily:"'Bebas Neue',cursive",fontSize:19,color:"#000",letterSpacing:3,cursor:"pointer",boxShadow:"0 4px 18px #FF6B3540"}}>⚡ GENERAR WOD</button>
  </div>;
}
...
export default function App() {
  ...
  return <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans',sans-serif",color:"#fff",display:"flex",flexDirection:"column"}}>
    ...
    <div style={{background:"#0a0a14",borderBottom:`1px solid ${C.border}`,padding:"9px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:28,height:28,background:"linear-gradient(135deg,#FF6B35,#FF2D55)",borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>🔥</div><div><div style={{fontFamily:"'Bebas Neue',cursive",fontSize:17,letterSpacing:3,lineHeight:1}}>WOD FORGE</div><div style={{color:C.dim,fontSize:7,letterSpacing:3}}>{session.user.email?.split("@")[0]?.toUpperCase()}</div></div></div>
    ...
    <div ref={ref} style={{flex:1,overflowY:"auto",padding:"16px 13px 90px",maxWidth:600,margin:"0 auto",width:"100%"}}>
      {tab==="generate"&&<div><div style={{marginBottom:20}}><div style={{fontFamily:"'Bebas Neue',cursive",fontSize:28,letterSpacing:3,lineHeight:1.05,marginBottom:4}}>GENERA TU<br/><span style={{color:C.accent}}>PRÓXIMO WOD</span></div><div style={{color:"#444",fontSize:11}}>Open · Semifinals · CrossFit Games · Rogue</div></div><GeneratorWizard onGenerate={cfg=>{setLastCfg(cfg);setWod(generateWOD(cfg));setTab("wod");ref.current?.scrollTo({top:0,behavior:"smooth"});}}/></div>}
