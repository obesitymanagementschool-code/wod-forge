    ...
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px",marginBottom:10}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{color,fontWeight:800,fontSize:12,letterSpacing:1}}>{BLOCK_TYPES.find(b=>b.id===block.kind)?.label||block.kind}</span>
        </div>
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        ...
        <button onClick={()=>onAddWodToDay(selDay)} style={{padding:"7px 14px",background:`linear-gradient(90deg,${C.flamingo},#FF2D7A)`,border:"none",borderRadius:7,fontFamily:"'Bebas Neue',cursive",fontSize:12,color:"#fff",cursor:"pointer",letterSpacing:1}}>+ ANADIR WOD</button>
      </div>
      {dayWods.length===0?<Placeholder icon="📅" text="Sin WODs para este dia"/>:dayWods.map((entry,i)=>{
        ...
        return (
          <div key={i} style={{background:C.card,border:`1px solid ${C.flamingo}30`,borderRadius:10,padding:"12px 14px",marginBottom:8}}>
            ...
            {wod?.blocks?.map((b,j)=><BlockCard key={j} block={b} color={C.flamingo}/>)}
          </div>
        );
      })}
    </div>
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:20}}>
        {[{l:"Dias / semana",v:daysThisWeek},{l:"Tiempo total (h)",v:(totalMins/60).toFixed(1)},{l:"sRPE medio",v:avgRpe}].map(s=>(
      ...
      <div style={{background:acwrColor+"15",border:`2px solid ${acwrColor}`,borderRadius:14,padding:"18px 20px",marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          <div style={{textAlign:"right"}}>
            <div style={{marginBottom:8}}><div style={{color:C.muted,fontSize:8,letterSpacing:2}}>CARGA AGUDA (7d)</div><div style={{fontFamily:"'Bebas Neue',cursive",fontSize:22,color:"#fff"}}>{acuteLoad}</div></div>
            <div><div style={{color:C.muted,fontSize:8,letterSpacing:2}}>CARGA CRONICA (28d avg)</div><div style={{fontFamily:"'Bebas Neue',cursive",fontSize:22,color:"#fff"}}>{chronicLoad}</div></div>
          </div>
        </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans',sans-serif",color:"#fff",display:"flex",flexDirection:"column"}}>
      ...
      <div ref={ref} style={{flex:1,overflowY:"auto",padding:"16px 13px 90px",maxWidth:600,margin:"0 auto",width:"100%"}}>
        ...
        {tab==="calendar"&&<CalendarView calendarWods={calendarWods} onStartWod={w=>setActiveTimer(w)} onAddWodToDay={date=>{if(wod){setShowSchedule(true);}else{setTab("generate");}}} onDeleteWod={async id=>{await sb.remove(session.token,"wod_calendar",id);await loadAll(session.token);}}/>}
